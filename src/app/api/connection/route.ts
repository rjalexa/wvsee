...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const serverInfo = `${metaData.version || "unknown"}|${
        metaData.hostname || "unknown"
      }`;
      const newConnectionId = connectionStore.generateConnectionId(
        `${url}|${serverInfo}`
      );
      ...
      // Store the connection ID and API key
      setConnectionId(newConnectionId);
      if (apiKey) {
        setWeaviateApiKey(apiKey);
      }
    } catch (error) {
      ...
      if (errorDetails.includes("ECONNREFUSED")) {
        errorHint =
          "This might be a Docker networking issue. If connecting to a container, make sure to use the internal port (e.g., :8080) not the host-mapped port (e.g., :8090).";
      } else if (errorDetails.includes("ENOTFOUND")) {
        errorHint =
          "Hostname could not be resolved. Make sure the container is on the same Docker network or use the container IP address.";
      } else if (errorDetails.includes("timeout")) {
        errorHint =
          "Connection timed out. Check if the Weaviate instance is running and accessible from this container.";
      }
    ...
    return NextResponse.json({
      ...
      url,
      newConnection:
        getConnectionId() !== currentConnectionId || !currentConnectionId,
    });
  } catch (error) {
